#!/usr/bin/env bash
# 8B Qwen3 Dense - Megatron BF16 baseline (no QAT)
set -xeuo pipefail
current_dir="$(dirname "$(readlink -f "$0")")"

project_name='DAPO-NVFP4-QAT'
exp_name=${exp_name:-"h100_8B_bf16_megatron_$(date +%m%d)"}
...
# BF16 baseline — explicitly disable QAT (dapo_qat_megatron_trainer.yaml defaults qat.enable=true).
QAT=(
