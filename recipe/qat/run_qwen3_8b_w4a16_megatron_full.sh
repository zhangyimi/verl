#!/usr/bin/env bash
# 8B Qwen3 Dense - Megatron W4A16 QAT, full
set -xeuo pipefail
current_dir="$(dirname "$(readlink -f "$0")")"

project_name='DAPO-NVFP4-QAT'
exp_name=${exp_name:-"h100_8B_w4a16_megatron_full_$(date +%m%d)"}
...
# QAT Configuration — W4A16 (FFN-only, attention untouched)
qat_enable=True
qat_mode=w4a16
...
# W4A16: force Marlin kernel (default flashinfer-cutlass always FP4-quantizes the input → W4A4 path)
...
QAT=(
    ...
    'actor_rollout_ref.actor.megatron.qat.ignore_patterns=["lm_head","*mlp.gate"]'
    ...
    '++actor_rollout_ref.rollout.qat.ignore_patterns=["lm_head","*mlp.gate"]'
)
