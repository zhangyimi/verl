...
def _modelopt_dense_process_weights(self, layer: torch.nn.Module) -> None:
    ...
    if _DENSE_PWAL_CALL_COUNT <= 3 or _DENSE_PWAL_CALL_COUNT % 100 == 0:
        print(
            f"[QAT-DEBUG] _modelopt_dense_process_weights call #{_DENSE_PWAL_CALL_COUNT} is_first={is_first_call} layer={type(layer).__name__} weight.shape={tuple(layer.weight.shape) if hasattr(layer, 'weight') else 'NO_WEIGHT'}",
            flush=True,
        )
...
def _modelopt_moe_marlin_convert(self, layer: torch.nn.Module, is_first_call: bool) -> None:
    ...
    w13_scale_2_processed = w13_scale_2_processed / w13_scale_factor
    w2_scale_2_processed = nvfp4_marlin_process_global_scale(layer.w2_weight_scale_2.data.to(param_dtype), param_dtype)
    w2_scale_2_processed = w2_scale_2_processed / w2_scale_factor
...
def prepare_modelopt_for_weight_reload(model, device=None):
    ...
    print(f"[QAT-DEBUG] prepare_modelopt_for_weight_reload ENTERED, model={type(model).__name__}", flush=True)
    inner_model = model
    if hasattr(model, "model"):
    ...
    print(f"[QAT-DEBUG] prepare_modelopt_for_weight_reload: cache size = {cache_size}", flush=True)
    ...
    for layer_name, cache_entry in param_meta._layer_meta_cache.items():
        ...
        for param_name, pm in cache_entry["meta"].items():
            ...
            restored_count += 1
    print(
        f"[QAT-DEBUG] prepare_modelopt_for_weight_reload: restored {restored_count} params (cache had {cache_size} layers)",
        flush=True,
    )
...
def apply_modelopt_nvfp4_patches(mode: str = "w4a4"):
    ...
    if mode == "w4a16":
        ...
        print("[QAT-DEBUG] set VLLM_NVFP4_GEMM_BACKEND=marlin (w4a16 mode)", flush=True)
