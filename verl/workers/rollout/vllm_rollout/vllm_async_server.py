...
class vLLMHttpServer:
    ...
    def _apply_quantization(self) -> tuple[Optional[str], dict]:
        ...
        print(
            f"[QAT-DEBUG] vllm_async_server.launch_server: self.config keys={list(self.config.keys()) if hasattr(self.config, 'keys') else type(self.config).__name__}",
            flush=True,
        )
        print(
            f"[QAT-DEBUG] qat_config_dict type={type(qat_config_dict).__name__} content={dict(qat_config_dict) if hasattr(qat_config_dict, 'keys') else qat_config_dict}",
            flush=True,
        )
        print(
            f"[QAT-DEBUG] qat_config_dict.get('enable', False) = {qat_config_dict.get('enable', False) if hasattr(qat_config_dict, 'get') else 'N/A'}",
            flush=True,
        )
        if qat_config_dict.get("enable", False):
            ...
            print(f"[QAT-DEBUG] forcing load_format=dummy (was {self.config.load_format})", flush=True)
