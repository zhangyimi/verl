...
class vLLMColocateWorkerExtension:
    ...
    def __new__(cls, **kwargs):
        ...
        print(
            f"[QAT-DEBUG] vLLMColocateWorkerExtension.__new__: vllm_config={vllm_config is not None}, quant_config type={type(quant_config).__name__ if quant_config else None}, quant_config repr={repr(quant_config)[:300] if quant_config else None}",
            flush=True,
        )
        if quant_config:
            print(
                f"[QAT-DEBUG] quant_config dir={[a for a in dir(quant_config) if not a.startswith('_')][:20]}",
                flush=True,
            )
            print(f"[QAT-DEBUG] quant_format={getattr(quant_config, 'quant_format', 'NO_ATTR')}", flush=True)
            print(f"[QAT-DEBUG] quant_method={getattr(quant_config, 'quant_method', 'NO_ATTR')}", flush=True)
        ...
        print(f"[QAT-DEBUG] _is_qat_model={_is_qat_model}, _is_modelopt_qat={_is_modelopt_qat}", flush=True)
        if _is_qat_model:
            ...
            print(f"[QAT-DEBUG] apply_modelopt_nvfp4_patches(mode={_qat_mode}) called in worker subprocess", flush=True)
            ...
            print(
                f"[QAT-DEBUG] ModelOptNvFp4LinearMethod.process_weights_after_loading is now: {pwal_name}", flush=True
            )
    ...
    def update_weights_from_ipc(self, peft_config: dict = None, base_sync_done=False, use_shm: bool = False):
        ...
        # DEBUG: instance-attribute survival check
        print(
            f"[QAT-DEBUG] update_weights_from_ipc: hasattr _is_modelopt_qat={hasattr(self, '_is_modelopt_qat')}, "
